...
Public Class Utilities
    ...
    Public Enum ControlPointPosition
        ...
        Below
    End Enum
    'Get the control point specified by the point flapping from and slope range
    Public Shared Function GetControlPointByLowPoint(ByVal ptFlappingFrom As PointInMem, ByVal minSlope As Double, ByVal maxSlope As Double, ByRef pointArray() As PointInMem, ByRef controlPoint As PointInMem) As ControlPointPosition
        Dim pos As ControlPointPosition = ControlPointPosition.Above
        Dim controlSlope As Double = maxSlope + 0.1
        Dim point As PointInMem
        For Each point In pointArray
            If Abs(point.Offset - ptFlappingFrom.Offset) < 0.00001 Then
                ' If first point is lower than reference point, the first point is control point
                If ptFlappingFrom.Elevation - point.Elevation > 0.00001 Then
                    controlPoint = point
                    Return ControlPointPosition.Below
                End If
            Else
                Dim slope As Double
                slope = (point.Elevation - ptFlappingFrom.Elevation) / Abs(point.Offset - ptFlappingFrom.Offset)
                If slope < maxSlope And slope < controlSlope Then
                    controlSlope = slope
                    controlPoint = point
                End If
            End If
        ...
        If controlSlope < maxSlope Then
            pos = ControlPointPosition.Inside
            If controlSlope < minSlope Then
                pos = ControlPointPosition.Below
            End If
        End If
        Return pos
    End Function

    Public Shared Function GetControlPointByHighPoint(ByVal ptFlappingFrom As PointInMem, ByVal minSlope As Double, ByVal maxSlope As Double, ByRef pointArray() As PointInMem, ByRef controlPoint As PointInMem) As ControlPointPosition
        Dim pos As ControlPointPosition = ControlPointPosition.Below
        Dim controlSlope As Double = minSlope - 0.1
        Dim point As PointInMem
        For Each point In pointArray
            If Abs(point.Offset - ptFlappingFrom.Offset) < 0.00001 Then
                ' If first point is higher than reference point, the first point is control point
                If ptFlappingFrom.Elevation - point.Elevation < -0.00001 Then
                    controlPoint = point
                    Return ControlPointPosition.Above
                End If
            Else
                Dim slope As Double
                slope = (point.Elevation - ptFlappingFrom.Elevation) / Abs(point.Offset - ptFlappingFrom.Offset)
                If slope > minSlope And slope > controlSlope Then
                    controlSlope = slope
                    controlPoint = point
                End If
            End If
        ...
        If controlSlope > minSlope Then
            pos = ControlPointPosition.Inside
            If controlSlope > maxSlope Then
                pos = ControlPointPosition.Above
            End If
        End If
        Return pos
    End Function

    Public Shared Function GetDesignSlopeFromEGHighLowPoint(ByVal ptFlappingFrom As PointInMem, ByVal minSlope As Double, ByVal maxSlope As Double, ByRef pointArray() As PointInMem, ByVal bHighPt As Boolean) As Double
        Dim dDesignSlope As Double
        Dim pos As ControlPointPosition
        Dim controlPoint As New PointInMem
        If bHighPt = True Then
            pos = GetControlPointByHighPoint(ptFlappingFrom, minSlope, maxSlope, pointArray, controlPoint)
        Else
            pos = GetControlPointByLowPoint(ptFlappingFrom, minSlope, maxSlope, pointArray, controlPoint)
        End If

        If pos = ControlPointPosition.Below Then ' control point is below min slope
            dDesignSlope = minSlope
        ElseIf pos = ControlPointPosition.Inside Then ' control point inside the slope range
            dDesignSlope = (controlPoint.Elevation - ptFlappingFrom.Elevation) / Abs(controlPoint.Offset - ptFlappingFrom.Offset)
        Else
            dDesignSlope = maxSlope
        End If
        Return dDesignSlope
    End Function

    'Get the control point which has minmum distance from specified point and fixed slope
    Public Shared Function GetControlPointByPointAndSlope(ByVal referencePoint As PointInMem, ByVal slope As Double, ByRef pointArray() As PointInMem, ByVal bLevel As Boolean, ByRef controlPoint As PointInMem) As Boolean
        Dim bPtsAtOneSide As Boolean = True
        If pointArray.Length = 0 Then
            Return bPtsAtOneSide
        End If

        Dim MiniClearance As Double = 99999.0#

        Dim bFirst As Boolean = True
        Dim pt As PointInMem
        For Each pt In pointArray
            Dim deltaElevation As Double
            deltaElevation = referencePoint.Elevation + Abs(pt.Offset - referencePoint.Offset) * slope - pt.Elevation
            If bLevel = False Then
                deltaElevation = -deltaElevation
            End If

            If bFirst = False Then
                If bPtsAtOneSide = True And deltaElevation * MiniClearance < -0.00001 Then
                    bPtsAtOneSide = False
                End If
            Else
                bFirst = False
            End If
            If (deltaElevation < MiniClearance) Then
                controlPoint.Offset = pt.Offset
                controlPoint.Elevation = pt.Elevation
                MiniClearance = deltaElevation
            End If
        Next
    ...
    Public Shared Function GetSuperSlope(ByVal vOverlaySlopeOption As Long, ByVal vSide As Long, ByVal oAlignment As Alignment, ByVal corridorState As CorridorState, ByVal dDefaultSlope As Double) As Double
        Dim type As SuperelevationCrossSegmentType
        If vOverlaySlopeOption = 2 And vSide = Utilities.Left Then
            type = SuperelevationCrossSegmentType.LeftOutLaneCrossSlope
        ElseIf vOverlaySlopeOption = 2 And vSide = Utilities.Right Then
            type = SuperelevationCrossSegmentType.RightOutLaneCrossSlope
        ElseIf vOverlaySlopeOption = 4 And vSide = Utilities.Left Then
            type = SuperelevationCrossSegmentType.LeftInLaneCrossSlope
        Else 'If vOverlaySlopeOption = 4 And vSide = Utilities.Right
            type = SuperelevationCrossSegmentType.RightInLaneCrossSlope
        End If

        Dim dSuperSlope As Double
        dSuperSlope = dDefaultSlope
        Try
            dSuperSlope = oAlignment.GetCrossSlopeAtStation(corridorState.CurrentStation, type, True)
        Catch
            Utilities.RecordWarning(corridorState, CorridorError.Failure, "Cross slope of lane superelevation not found. Using ideal cross slope instead.", "Overlay_Level_1Lane_1Side")
        End Try
        Return dSuperSlope
    End Function

    Public Shared Function GetDesignSlopeFromOverlaySlopeOption(ByVal vOverlaySlopeOption As Long, ByVal dDefaultSlope As Double, ByVal dExistSlope As Double, dSuperSlope As Double, ByVal dSlopeTolerance As Double) As Double
        Dim dDesignSlope As Double
        If vOverlaySlopeOption = 1 Then  'MatchSlope
            dDesignSlope = dExistSlope
        ElseIf vOverlaySlopeOption = 5 Then 'UseDefaultSlope
            dDesignSlope = dDefaultSlope
        ElseIf vOverlaySlopeOption = 3 Then 'UserDefinedWithoutFlapping
            dDesignSlope = dDefaultSlope
            If Abs(dDefaultSlope - dExistSlope) > dSlopeTolerance Then
                If (dExistSlope < dDefaultSlope) Then
                    dDesignSlope = dDefaultSlope - dSlopeTolerance
                Else
                    dDesignSlope = dDefaultSlope + dSlopeTolerance
                End If
            End If
        ElseIf vOverlaySlopeOption = 0 Then 'UserDefinedWithFlapping
            'change the behavior in from 2019.3 and 2020.1, comment the flapping havior
            'dDesignSlope = Utilities.GetDesignSlopeFromEGHighLowPoint(ptFlappingFrom, (dDefaultSlope - dSlopeTolerance), (dDefaultSlope + dSlopeTolerance), ptArrForFlapping, True)
            dDesignSlope = dExistSlope
            If Abs(dDefaultSlope - dExistSlope) > dSlopeTolerance Then
                If (dExistSlope < dDefaultSlope) Then
                    dDesignSlope = dExistSlope + dSlopeTolerance
                Else
                    dDesignSlope = dExistSlope - dSlopeTolerance
                End If
            End If
        Else 'Outside Lane Super-elevation & Inside Lane Super-elevation
            dDesignSlope = dSuperSlope
            If Abs(dSuperSlope - dExistSlope) > dSlopeTolerance Then
                If (dExistSlope < dSuperSlope) Then
                    dDesignSlope = dSuperSlope - dSlopeTolerance
                Else
                    dDesignSlope = dSuperSlope + dSlopeTolerance
                End If
            End If
    ...
    Public Property UseFeature As SubBaseDirection
        Get
            Return _useFeature
        End Get
        Set(value As SubBaseDirection)
            _useFeature = value
        End Set
    End Property
    Private _useFeature As SubBaseDirection
